...
@Autonomous(name = "Blue Far Options - CTS", group = "Autonomous", preselectTeleOp = "tele - CTS")
public class BlueFarOptionsCTS extends OpMode {
    ...
    private boolean goToSpike3 = true;   // configurable in init_loop
        ...
        switch (pathState) {
            ...
            case 1:
                ...
                if (shouldOpenGate()) {
                    ...
                    hw.intake.intake();
                    if (goToSpike3) {
                        // Driver chose to collect spike 3 first
                        follower.followPath(driveToSpike3, true);
                        setPathState(2);
                    } else {
                        // Driver chose to skip spike 3 — go straight to HP zone
    ...
    public void init_loop() {
        ...
        // Toggle spike 3 collection with X button
        if (gamepad1.xWasPressed()) goToSpike3 = !goToSpike3;

        String sequence = goToSpike3
                ? "Preload → Spike 3 → HP zone (repeating)"
                : "Preload → HP zone (repeating, no spike 3)";

        telemetry.addLine("Alliance: Blue  |  Position: Back Wall");
        telemetry.addLine("Sequence: " + sequence);
        telemetry.addLine("Spike 3: " + (goToSpike3 ? "YES  [X] to skip" : "SKIP [X] to include"));
